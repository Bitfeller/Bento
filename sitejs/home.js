...
function show(deck) {
    ...
    let worst = keys.filter(k => review[k].box <= 2).sort((a, b) => review[a].box + review[a].score / 100 - review[b].box - review[b].score / 100).slice(0, INFO_TERM_LIMIT).map(k => `<li>${deck.data.contnt[k] ? k : ""} => ${deck.data.contnt[k].type == 'mc' ? deck.data.contnt[k].ans.map(r => deck.data.contnt[k].op[r]).join(', ') : deck.data.contnt[k].ans} (recall rating: ${review[k].box} (of 6), score: ${review[k].score}, next review: ${UserGateway.calculateNTR(review[k].box, review[k].last) ? "now" : "in " + UserGateway.calculateNextReview(review[k].box, review[k].last) + " day(s)"})</li>`).join('');
    let learning = keys.filter(k => review[k].box > 2 && review[k].box < 5).sort((a, b) => review[a].box + review[a].score / 100 - review[b].box - review[b].score / 100).slice(0, INFO_TERM_LIMIT).map(k => `<li>${deck.data.contnt[k] ? k : ""} => ${deck.data.contnt[k].type == 'mc' ? deck.data.contnt[k].ans.map(r => deck.data.contnt[k].op[r]).join(', ') : deck.data.contnt[k.replace('&amp;', '&')].ans} (recall rating: ${review[k].box} (of 6), score: ${review[k].score}, next review: ${UserGateway.calculateNTR(review[k].box, review[k].last) ? "now" : "in " + UserGateway.calculateNextReview(review[k].box, review[k].last) + " day(s)"})</li>`).join('');
